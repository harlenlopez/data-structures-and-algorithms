...
const returnNames = (arr) => {
  return arr.reduce((answer, characters) => {
    answer.push(characters.name);
    return answer;
  }, [])
};
...
const countNumberOfChildren = (arr) => {
  return arr.reduce((answer, value) => {
    return value.child ? answer + value.child.length :answer;
  }, 0)
};
...
const countPrimeNumbers = (arr) => {
  return arr.reduce((acc, potato) => acc + (isPrime(potato)))
  return ;
};
